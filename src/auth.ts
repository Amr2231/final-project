...
//         const first =
//           session.firstName ??
//           (session as { first_name?: string }).first_name;
//         const last =
...
//       // ── 3. حدّث الـ refreshToken لو عندنا أحدث ────────────────
...
//         const newExpiresAt = Date.now() + 1 * 60 * 1000; // 1 minutes
...
export const authOptions: NextAuthOptions = {
  session: {
    strategy: "jwt",
    // أقصى مدة ممكنة — التحكم الفعلي بيتم في الـ cookie عن طريق session-preference route
    maxAge: 7 * 24 * 60 * 60,
  },
  ...
  callbacks: {
    ...
    jwt: async ({ token, user, trigger, session }) => {
      ...
      if (user) {
        ...
        token.username = user.username;
        token.expiresAt = Date.now() + 15 * 60 * 1000;
        // ← سجّل وقت الـ login عشان نحسب مدة السيشن
      ...
      // ── 4. لو rememberMe = false وعدت ساعة → اطرده ──────────
      if (!token.rememberMe) {
        const SESSION_LIMIT = 60 * 60 * 1000; // ساعة واحدة
      ...
      try {
        ...
        const newExpiresAt = Date.now() + 15 * 60 * 1000; // 15 دقيقة
