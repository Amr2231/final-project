...
// تحقق إن السيشن لسه في المدة المسموح بيها
function isSessionExpiredByAge(token: {
  rememberMe?: unknown;
  loginAt?: unknown;
}): boolean {
  // لو rememberMe = true → السيشن تقعد 7 أيام (NextAuth بيتحكم فيها)
  if (token.rememberMe) return false;

  // لو rememberMe = false → السيشن تقعد ساعة بس
  const SESSION_LIMIT = 60 * 60 * 1000; // ساعة بالمللي ثانية
...
export async function proxy(req: NextRequest) {
  ...
  // token expired (من NextAuth نفسه)
  if (token?.error === "RefreshTokenExpired") {
    const res = NextResponse.redirect(new URL("/login", req.nextUrl.origin));
    return withCSP(clearSession(res));
  }
  ...
  // ← السيشن عدت ساعة وmemberMe = false
